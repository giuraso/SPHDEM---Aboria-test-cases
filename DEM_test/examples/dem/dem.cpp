...
*/
    ...
    freopen("../velocity.txt","w",stdout);

params->dem_diameter = 0.001;
params->dem_gamma = 0.01;
params->dem_k = 0.0;
...
params->dem_dt = 1e-6;//(1.0/50.0)*PI/sqrt(params->dem_k/dem_min_reduced_mass-pow(0.5*params->dem_gamma/dem_min_reduced_mass,2));
...
*/
dem->create_particles(1,[params,ndem,L,&dice](DemType::Value& i) {
Vect3d& v = std::get<DEM_VELOCITY>(i.get_data());
Vect3d& f = std::get<DEM_FORCE>(i.get_data());

v << 1,0,0;
f << 0,0,0;
Vect3d position(L*0.45,L*0.5,L*0.5);
return position;
});
...
v << -1,0,0;
f << 0,0,0;
Vect3d position(L*0.55,L*0.5,L*0.5);
return position;
});
