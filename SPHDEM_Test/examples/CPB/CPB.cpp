...
dem->init_neighbour_search(min,max,params->dem_diameter,periodic);
	...
	for (int i = 0; i < nout; ++i) {
		...
		myfile << v[2] <<" "<< f0[0] <<" "<< f0[1] <<" "<< f0[2];
		myfile << r[0] <<" "<< r[1] <<" "<< r[2] << endl;
		}
...
*/
	double td=params->dem_mass/(3*PI*params->dem_diameter/params->sph_visc);
	std::cout<<td;
	myfile.close();
}
