...
enum {DEM_FORCE, DEM_DIAMETER, DEM_VELOCITY, DEM_VELOCITY0, DEM_FORCE0,DEM_SHEPSUM};
typedef std::tuple<Vect3d,double,Vect3d,Vect3d,Vect3d,double> DemTuple;
...
#define REGISTER_DEM_PARTICLE(particle) \
				...
				GET_TUPLE(double,dp,DEM_DIAMETER,particle);\
...
#define REGISTER_NEIGHBOUR_DEM_PARTICLE(tuple) \
				...
				const GET_TUPLE(double,dpj,DEM_DIAMETER,j);\
...
enum {SPH_FORCE, SPH_VELOCITY, SPH_VELOCITY0,SPH_FORCE0,SPH_DENS, SPH_POROSITY, SPH_H, SPH_DDDT,SPH_PDR2,SPH_OMEGA,SPH_FIXED,SPH_VOL_A,SPH_VOL_B,SPH_FORCE_EXT};
typedef std::tuple<Vect3d,Vect3d,Vect3d,Vect3d,double,double,double,double,double,double,bool,double,double,Vect3d> SphTuple;
...
#define REGISTER_SPH_PARTICLE(particle) \
				...
				GET_TUPLE(double,VolumepartA,SPH_VOL_A,particle); \
				GET_TUPLE(double,VolumepartB,SPH_VOL_B,particle); \
...
#define REGISTER_NEIGHBOUR_SPH_PARTICLE(tuple) \
				...
				const GET_TUPLE(double,VolumepartAj,SPH_VOL_A,j); \
				const GET_TUPLE(double,VolumepartBj,SPH_VOL_B,j); \
...
struct Params {
	...
	double randomize_end, dem_diameter_big, dem_diameter_small, randomize_time, dem_dens, dem_gravity, dem_dt,dem_k,dem_gamma, dem_vol, dem_time_drop;
	double time;
};
...
void dem_timestep(ptr<DemType> dem,
	...
	std::for_each(dem->begin(),dem->end(),[&geometry,dem,dem_k,dem_gamma,dem_dens](DemType::Value& i) {
		REGISTER_DEM_PARTICLE(i);
		...
		for (auto tpl: i.get_neighbours(dem)) {
			...
			const double overlap = dp-r;
			if (overlap>0) {
				const Vect3d normal = dx/r;
				const Vect3d dv = v-vj;
				double dem_mass=pow(dp,3)*PI/6*dem_dens;
...
const double dem_dens = params->dem_dens;
bool onlyonce=true;
if ((params->time < params->dem_time_drop) && (params->time < params->randomize_time)){
return;
}
else{
	if ((params->time < params->dem_time_drop) && (params->time > params->randomize_time) && (params->time < params->randomize_end)){
	params->dem_gravity=0.0;
	}
	
	else{
                if ((params->time < params->dem_time_drop) && (params->time > params->randomize_end) && (onlyonce)){
		onlyonce=false;
...
void sphdem(ptr<SphType> sph,ptr<DemType> dem,
	...
	const double dem_diameter_big=params->dem_diameter_big;
	const double dem_diameter_small=params->dem_diameter_small;
	const double dem_dens= params->dem_dens;
...
Vect3d accel(0,0,0);
	...
	std::for_each(dem->begin(),dem->end(),[&accel,params,sph,dem_dens,sph_mass,sph_visc,sph_dens](DemType::Value& i) {
		REGISTER_DEM_PARTICLE(i);
		...
		ef /= s;
			...
			double Re=vmod*dp/sph_visc;
			double Cd=pow((0.63+4.8/sqrt(Re)),2);
			double feps=pow(ef,-(3.7-0.65*exp(-(pow(1.5-log10(Re),2)/2))));
			Vect3d fdrag = 1/8.0*Cd*feps*PI*pow(dp,2)*vmod*vs; //DallaValle DI Felice
...
//			const Vect3d fdrag = 3.0*PI*sph_visc*dem_diameter*ef*(vf-v); //Stokes
			double dem_vol=pow(dp,3)*PI/6;
			double dem_mass=dem_vol*dem_dens;
	//	std::cout<<dem_vol<<" "<<dem_mass<<endl;
			f0 = sph_dens*(dem_vol*f0 + fdrag)/dem_mass;
	//		std::cout <<"v = "<<v<<" vf = "<<vf<<" f0 = "<<f0<<" fdrag = "<<fdrag<<"v / m = "<<dem_vol/dem_mass<<std::endl;
	...
	std::for_each(sph->begin(),sph->end(),[dem, dem_diameter_big](SphType::Value& i) {
		REGISTER_SPH_PARTICLE(i);
		for (auto tpl: i.get_neighbours(dem)) {
			REGISTER_NEIGHBOUR_DEM_PARTICLE(tpl);
			const double r2 = dx.squaredNorm();
			...
			const double r = sqrt(r2);
			double dem_vol=pow(dpj,3)*PI/6;
			if (dpj==dem_diameter_big)			
				VolumepartA += dem_vol*W(r/h,h);
			else			
				VolumepartB += dem_vol*W(r/h,h);
	...
	std::for_each(dem->begin(),dem->end(),[sph,sph_mass](DemType::Value& i) {
		REGISTER_DEM_PARTICLE(i);
		s = 0;
		for (auto tpl: i.get_neighbours(sph)) {
			REGISTER_NEIGHBOUR_SPH_PARTICLE(tpl);
	...
	std::for_each(sph->begin(),sph->end(),[dem,dem_dens](SphType::Value& i) {
		REGISTER_SPH_PARTICLE(i);
		...
		for (auto tpl: i.get_neighbours(dem)) {
			...
			if (r2 > 4.0*h*h) continue;
			const double r = sqrt(r2);
			double dem_mass=pow(dpj,3)*PI/6*dem_dens;
			f0 -= dem_mass*f0j*W(r/h,h)/sj;
		}
		f0 /= rho;
	});
	double diam_big=params->dem_diameter_big;
	 
	dem->reset_neighbour_search(diam_big);
